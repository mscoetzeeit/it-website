# -*- coding: utf-8 -*-
"""
Build three print-ready, black-and-white booklets (booklet-gr10.html,
booklet-gr11.html, booklet-gr12.html) from the content pages of the
Ms Coetzee IT website - one booklet per grade, instead of a single
combined document.
...
- Adds a cover page and a table of contents at the FRONT of each grade's
  booklet, scoped to that grade only.
- Appends that grade's Year Planner and the shared Exam Extras page as the
  final chapters of each booklet.
- Ships a light, B&W print stylesheet that also recolours the inline dark-theme
  SVG diagrams to dark-on-white so they print cleanly, with widow/orphan
  control so headings are never stranded alone at the bottom of a page.
"""
...
# (sub, relative path, chapter title)  -- order = nav.js
GRADE_PAGES = {
    "10": {
        "colour": "#0891b2",
        "chapters": [
            ...
            ("Term Planner", "terms-gr10.html",                       "Grade 10 - By Term"),
            ("Exam Resources", "exam-extras.html",                    "Exam Extras (Out-of-CAPS)"),
        ],
    },
    "11": {
        "colour": "#6d28d9",
        "chapters": [
            ...
            ("Term Planner", "terms-gr11.html",                        "Grade 11 - By Term"),
            ("Exam Resources", "exam-extras.html",                     "Exam Extras (Out-of-CAPS)"),
        ],
    },
    "12": {
        ...
        "chapters": [
            ...
            ("Term Planner", "terms-gr12.html",                "Grade 12 - By Term"),
            ("Exam Resources", "exam-extras.html",             "Exam Extras (Out-of-CAPS)"),
        ],
    },
}
...
def build_grade(grade, info):
    ...
    with io.open(os.path.join(ROOT, out_name), "w", encoding="utf-8") as fh:
        fh.write(out)
    print("Wrote %s with %d chapters." % (out_name, len(info["chapters"])))


def build():
    for grade, info in GRADE_PAGES.items():
        build_grade(grade, info)
...
<title>Ms Coetzee IT - Grade {{GRADE}} Study Reference (Printable Booklet)</title>
<style>
...
/* ---- TABLE OF CONTENTS (front of this grade's booklet) ---- */
...
/* ---- WIDOW/ORPHAN CONTROL FOR HEADINGS ----
   A heading must never be left alone as the last line on a page with its
   own content pushed to the next page. break-after:avoid-page forces the
   heading to move down WITH whatever follows it, rather than being
   orphaned. break-inside:avoid-page stops a heading's own text (when it
   wraps to two lines) from being split across a page boundary either. */
...
/* General widow/orphan control for body text: never leave a single stray
   line of a paragraph alone at the top or bottom of a printed page. */
...
/* ---- HARD-BLOCK BANNER ---- solid black/white for "cannot be used/assessed"
   warnings, since colour (red) would just print as grey on a B&W printer. */
.hard-block{ background:#111; color:#fff; font-weight:800; text-align:center; padding:.55em .9em; border-radius:4px; margin:.5em 0 1em; font-size:10pt; page-break-inside:avoid; break-inside:avoid-page; }
...
/* ---- CODE ---- */
.code-block{ margin:.9em 0; page-break-inside:avoid; break-inside:avoid-page; }
...
.hw-card{ border:1px solid var(--rule); border-radius:6px; padding:.5em; text-align:center; page-break-inside:avoid; break-inside:avoid-page; }
...
.delphi-panel{ border:1px solid var(--rule); border-radius:6px; overflow:hidden; margin:.8em 0; page-break-inside:avoid; break-inside:avoid-page; }
...
.extra-card{ border:1px solid var(--rule); border-left:3px solid var(--grade); border-radius:0 4px 4px 0; padding:.6em .8em; page-break-inside:avoid; break-inside:avoid-page; }
...
<div class="sheet">
  ...
  <section class="cover">
    ...
    <p class="sub">A complete printable guide to Information Technology for Grade {{GRADE}} - practical Delphi programming and theory.</p>
    <div class="grades">Practical &nbsp;&bull;&nbsp; Theory &nbsp;&bull;&nbsp; Term Planner &nbsp;&bull;&nbsp; Exam Extras</div>
